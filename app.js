...
document.getElementById("onboarding-form").addEventListener("submit", (e) => {
  e.preventDefault();
  const eurInput = document.getElementById("start-eur");
  const rsdInput = document.getElementById("start-rsd");
  const eurValue = parseFloat(eurInput.value);
  const rsdValue = parseFloat(rsdInput.value);

  if (!(eurValue > 0) && !(rsdValue > 0)) {
    alert("Укажите стартовый баланс хотя бы в одной валюте.");
    return;
  }
